...
盤の境界はトーラス(上下左右がつながった輪環)と有限盤を切り替えられ、ルールはプリセットから選ぶほかB/S記法を直接入力できる。今の盤が静物・振動子(周期つき)・消滅のどれに落ち着くかを調べる解析、状態をそのままURLに載せる共有、盤面をRLE形式で書き出す機能、LifeWikiなどのRLEを貼り付けて取り込む読み込み、今見えている盤や時空図をPNG画像として保存する書き出しを備える。配色はライトとダークに対応し、再生速度・グリッド線の有無は記憶される。動かさずに1世代ずつ進めることも、進めすぎたら1世代ずつ戻すことも、キーボードだけで操作することもできる。
...
上部のセグメントで「ライフ系」(2次元)と「基本CA」(1次元)を切り替える。再生・一時停止、1世代進める / 戻す、消去、ランダム配置、速度の調整は中央のトランスポートにまとまっている。主なキーボード操作は次のとおり。ヘッダの「?」ボタンか `?` キーでこの一覧をいつでも開ける。
...
2次元モードでは盤をなぞって生きたセルを描き、ツールを「置く」にすると選んだパターン(グライダー、パルサー、ゴスパーの銃など)を盤に置ける。ルールはプリセットの選択か、B/S記法(例 `B3/S23`)の直接入力で変えられる。「判定」を押すと今の盤が静物・振動子・消滅のどれに向かうかを調べる。「共有」で状態を載せたリンクをコピーし、「読込」でRLEを貼り付けて盤に取り込み、「RLE」で盤面をRLE形式で書き出し、「画像」で今見えている盤や時空図をPNGとして保存する。再生は requestAnimationFrame に乗せて速度を一定に保ち、戻る操作は直前の世代まで遡れる。
